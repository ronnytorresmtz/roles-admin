<?php namespace MyCode\Services\Validation;
	...
	public function validateInputs($model, $request, $form, $messages) {
		...
		if ($validator->fails()) {
        	foreach ($validator->errors()->all() as $errorMessage) {
				$message = array('error' => true, 'message' => $errorMessage, 400);
			}
        } else{
        	$message = array('error' => false, 'message' => Lang::get('messages.success'), 200);
        }
